...
class enrol_hubtel_testcase extends advanced_testcase {
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['hubtel']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('hubtel'));
        $plugin = enrol_get_plugin('hubtel');
        $this->assertInstanceOf('enrol_hubtel_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_hubtel', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $this->enable_plugin();
        $hubtelplugin = enrol_get_plugin('hubtel');

        // Just make sure the sync does not throw any errors when nothing to do.
        $hubtelplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_hubtel_plugin $hubtelplugin  */
        $hubtelplugin = enrol_get_plugin('hubtel');
        ...
        $hubtelplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $hubtelplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $hubtelplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $hubtelplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $hubtelplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $hubtelplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $hubtelplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $hubtelplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $hubtelplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $hubtelplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $hubtelplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $hubtelplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $hubtelplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
